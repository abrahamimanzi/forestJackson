		...
		<div class="row">
			...
			<form action="" method="POST">
				<div class="col-md-3">
					<h4>Property Type </h4>
					<label>
						<input name="property_type" type="checkbox" value="Apartment">  Apartment
					</label>
					<br>
					<label>
						<input name="property_type" type="checkbox" value="Compound"> Compound
					</label>
					<br>
					<label>
						<input name="property_type" type="checkbox" value="Duplex - attached"> Duplex - attached
					</label>
					<br> 
					<label>
						<input name="property_type" type="checkbox" value="Duplex - semidetached"> Duplex - semidetached
					</label>
					<br>
					<label>
						<input name="property_type" type="checkbox" value="Multi-family"> Multi-family
					</label>
					<br>
					<label>
						<input name="property_type" type="checkbox" value="Townhouse"> Townhouse
					</label>
					<br>
					<label>
						<input name="property_type" type="checkbox" value="Single family"> Single family
					</label>
					<br>
					<label>
						<input name="property_type" type="checkbox" value="Single storey"> Single storey
					</label>
				</div>
				<div class="col-md-3">
					<h4>Property Details </h4>
					<label>
						<input name="property_details" type="checkbox" value="_noo_property_feature_tv-room-upstairs"> Additional tv/reading room
					</label>
					<br>
					<!-- <label>
						<input name="property_details[]" type="checkbox" value="Air conditioning"> Air conditioning
					</label>
					<br> -->
					<label>
						<input name="property_details" type="checkbox" value="_noo_property_feature_bars-on-windows"> Bars on windows downstairs
					</label>
					<br>
					<label>
						<input name="property_details" type="checkbox" value="_noo_property_feature_bars-on-windows"> Bars on windows upstairs
					</label>
					<br>
					<!-- <label>
						<input name="property_details[]" type="checkbox" value="Corner house"> Corner house
					</label>
					<br> -->
					<label>
						<input name="property_details" type="checkbox" value="_noo_property_feature_covered-parking"> Covered parking</label>
					<!-- <br>
					<label>
						<input name="property_details[]" type="checkbox" value="Open floor plan"> Open floor plan
					</label> -->
				</div>
				<div class="col-md-3">
					<h4> Additional Features </h4>
					<label>
						<input name="property_features" type="checkbox" value="_noo_property_feature_dishwasher"> Dishwasher
					</label>
					<br>
					<label>
						<input name="property_features" type="checkbox" value="_noo_property_feature_generator"> Generator</label>
					<br>
					<label>
						<input name="property_features" type="checkbox" value="_noo_property_feature_swimming-pool"> Pool - individual</label>
					<br>
					<label>
						<input name="property_features" type="checkbox" value="_noo_property_feature_swimming-pool"> Pool - shared
					</label>
					<br>
					<label>
						<input name="property_features" type="checkbox" value="_noo_property_feature_washing-machine"> Washing machine
					</label>
					<br>
					<label>
						<input name="property_features" type="checkbox" value="_noo_property_feature_water-tank-pump"> Water pump
					</label>
					<br>
					<label>
						<input name="property_features" type="checkbox" value="_noo_property_feature_water-tank-pump">  Water tank
					</label>
				</div>
				<div class="col-md-3">
					<h4> Other</h4>
					<label>
						<input name="property_other" type="checkbox" value="Available short - term"> Available short - term
					</label>
				</div>
				<div class="col-md-2 col-xs-12 form-left-agileits-submit">
					<input type="submit" value="Submit" style="margin-top: 0;">
				</div>
				<input type="text" name="searchadvanced" hidden>
			</form>
		</div>
	</div>
</div>
				...
				while($row = $fetch = $terms->fetch(PDO::FETCH_ASSOC)) {
					...
					while($row = $fetch = $term->fetch(PDO::FETCH_ASSOC)) {
						...
						<?php
							...
							/*
							// $qu = $db->pdo->prepare('SELECT * FROM `wpbb_postmeta` WHERE `meta_key`="'.$property_details.'" AND `meta_value`=1 AND `meta_key` LIKE "%_noo_property_feature%" ORDER BY `meta_id` DESC limit 10');
							// $qu->execute();
							// while($row = $fetch = $qu->fetch(PDO::FETCH_ASSOC)) {
							// 	$meta_id = $row['meta_id'];
							...
							$prop = $db->pdo->prepare('SELECT * FROM `wpbb_posts` WHERE `ID`IN (SELECT post_id FROM wpbb_postmeta WHERE `meta_key`="'.$property_details.'" AND `meta_value`=1 AND `meta_key` LIKE "%_noo_property_feature%" ORDER BY `meta_id`) AND `post_type`="noo_property" AND `post_status`="publish" AND ID IN (SELECT object_id FROM wpbb_term_relationships WHERE object_id IN (SELECT post_id FROM wpbb_postmeta WHERE `meta_key`="'.$property_details.'" AND `meta_value`=1 AND `meta_key` LIKE "%_noo_property_feature%" ORDER BY `meta_id`) ) ORDER BY `ID`  DESC ');




							// $prop = $db->pdo->prepare('SELECT * FROM `wpbb_posts` WHERE `ID`="'.$post_id.'" AND `post_type`="noo_property" AND `post_status`="publish" AND ID IN (SELECT object_id FROM wpbb_term_relationships WHERE object_id = "'.$post_id.'" ) ORDER BY `ID` ');
				            // $prop = $db->pdo->prepare('SELECT * FROM `wpbb_posts` WHERE `ID`="'.$object_id.'" AND `post_type`="noo_property" AND `post_status`="publish" ORDER BY `ID` DESC limit 12');
							$prop->execute();
							while($row = $fetch = $prop->fetch(PDO::FETCH_ASSOC)) {
								$p_id = $row['ID'];
...
<?php if(!isset($_POST['search']) AND !isset($_POST['searchsales']) AND !isset($_POST['searchadvanced']) ){ ?>
<div class="container">
	<div class="row">
		<div class="col-md-12">
			<h2 class="titles">Featured Residential Properties</h2>
		</div>         
	</div>
</div>
<!-- about section -->
<section id="#">
  	<div class="container">
		<?php
			...
			$start = ($curpage * $perpage) - $perpage;
            $PageSql = $db->pdo->prepare("SELECT * FROM `wpbb_posts` WHERE `post_type`='noo_property' AND `post_status`='publish'");
            $PageSql->execute();
            $totalres = $PageSql->rowCount();
            $endpage = ceil($totalres/$perpage);
			$startpage = 1;
            ...
            $query = $db->pdo->prepare("SELECT * FROM `wpbb_posts` WHERE `post_type`='noo_property' AND `post_status`='publish' ORDER BY `ID` DESC LIMIT $start, $perpage");
            $query->execute();
        ...
        <div class="row no-margin">
         ...
         <h3 style="color: #9b635e;">DISCLAIMER</h3>
          <p style="font-weight: normal;">
            Information deemed reliable but is not guaranteed.<br/>
            All information relating to real estate on this website comes from manual real estate searches across the country or from landlords and sellers submitting information to Forrest Jackson Properties. <br/>
