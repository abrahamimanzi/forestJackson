...
<section id="#">
  	<div class="container">
		<?php
            ...
            $query = $db->pdo->prepare("SELECT * FROM `wpbb_posts` WHERE `post_type`='noo_property' AND `post_status`='publish' ORDER BY `ID` DESC limit 12");
            $query->execute();
       						...
       						while($row = $fetch = $qu->fetch(PDO::FETCH_ASSOC)) {
	        					...
	        					$qu->execute();
	       						while($row = $fetch = $qu->fetch(PDO::FETCH_ASSOC)) {
	       							$img=$row['guid'];
              			?>
                		<!-- <img src="img/properties/<?=$image?>" alt="picture-01" class="rule"> -->
